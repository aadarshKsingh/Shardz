...
export const SideBar = () => {
  ...
  return (
    <div className="flex flex-row -px-20">
      <Sidebar className="lg:pr-0 pr-0">
        <Sidebar.Items>
          <Sidebar.ItemGroup>
            <Sidebar.Item>
            ...
            <Sidebar.Item icon={LuLayoutDashboard}>
              <p onClick={() => navigate("/dashboard")} className="pr-20">
                Dashboard
              </p>
            </Sidebar.Item>
            ...
            <Sidebar.Item
              icon={LuLayers}
              labelColor="dark"
              label={addedDrives === null ? 0 : addedDrives.length}
            >
              <p onClick={() => navigate("/drives")} className="pr-20">
                Drives
              </p>
            </Sidebar.Item>
          </Sidebar.ItemGroup>
          <Sidebar.ItemGroup>
            <Sidebar.Item className="bg-[#5793FB] bg-opacity-20 scale-75 lg:scale-100 lg:mx-0 -mx-8">
              <span className="flex flex-row -mx-3">
                <LuHardDrive className="h-7 w-7 opacity-60 py-50" />
                <span className="pl-2">Storage</span>
              </span>
              <span className="font-thin text-sm pt-3 pb-2">
                {byte2gig(sessionStorage.getItem("remaining"))} GB of{" "}
                {byte2gig(sessionStorage.getItem("total"))} GB used
              </span>
              <span className="pb-5">
                <Progress
                  progress={calculateStoragePercentage(
                    sessionStorage.getItem("total") -
                      sessionStorage.getItem("remaining"),
                    sessionStorage.getItem("total")
                  )}
                  color="blue"
                />
